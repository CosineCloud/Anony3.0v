...
import uuid
import operations
from process_broadcasting import process_broadcasting_channel_id
...
def handle_anony_number_callback(call):
    bot.answer_callback_query(
    call.id,
    text="⚠️\n\nYour Anony Number is permanent and completely safe to share for frequent commnications! ",
    show_alert=True
)
...
def handle_broadcasting_callback(call):
    ...
    try:
        ...
        cursor.execute("SELECT STATUS FROM users WHERE USER_ID = ?", (call.from_user.id,))
        status_data = cursor.fetchone()
        
        if status_data:
            current_status = status_data[0]
            
            # Check if user is already in a service
            if current_status in ["LISTENER", "BROADCASTER", "AI", "CONNECTED", "RANDOM", "PRIVATE"]:
                # Show popup message
                bot.answer_callback_query(
                    call.id,
                    text="Please close the current service",
                    show_alert=True
                )
                return
        
        # Create inline keyboard with Listener and Broadcaster options
        markup = telebot.types.InlineKeyboardMarkup()
        markup.row(
            telebot.types.InlineKeyboardButton("🎧 Listener", callback_data="broadcasting_listener"),
            telebot.types.InlineKeyboardButton("🎙️ Broadcaster", callback_data="broadcasting_broadcaster")
        )
        markup.row(telebot.types.InlineKeyboardButton("❌ Cancel", callback_data="broadcasting_cancel"))
...
@bot.callback_query_handler(func=lambda call: call.data == "broadcasting_listener")
def handle_broadcasting_listener_callback(call):
    """Handle the Listener option for broadcasting."""
    try:
        # Replace the options message with the input prompt
        msg = bot.edit_message_text(
            "ℹ️ Please enter the broadcasting channel ID starting with 'BCST':",
            call.message.chat.id,
            call.message.message_id
        )
        # Register the next step handler with a lambda function to pass the bot instance
        bot.register_next_step_handler(msg, lambda message: process_broadcasting_channel_id(bot, message, call.from_user.id))
    except Exception as e:
        logger.error(f"Error handling broadcasting listener option: {e}")
        bot.send_message(call.message.chat.id, "Error processing request. Please try again later.")

# This function has been moved to process_broadcasting.py

@bot.callback_query_handler(func=lambda call: call.data == "broadcasting_broadcaster")
def handle_broadcasting_broadcaster_callback(call):
    """Handle the Broadcaster option for broadcasting."""
    try:
        # Get user's ANONY_NAME from the database
        cursor.execute("SELECT ANONY_NAME FROM users WHERE USER_ID = ?", (call.from_user.id,))
        anony_name_data = cursor.fetchone()
        
        if not anony_name_data or not anony_name_data[0]:
            # Generate a new ANONY_NAME if not found
            anony_name = generate_anony_name()
            cursor.execute("UPDATE users SET ANONY_NAME = ? WHERE USER_ID = ?", (anony_name, call.from_user.id))
            conn.commit()
        else:
            anony_name = anony_name_data[0]
        
        # Import the broadcasting module
        import create_broadcasting
        
        # Generate broadcasting channel ID directly
        channel_id = create_broadcasting.create_broadcasting_channel_id(
            call.from_user.id, 
            anony_name
        )
        ...
        if not channel_id:
            bot.edit_message_text(
                "Error generating broadcasting channel ID. Please try again.",
                call.message.chat.id,
                call.message.message_id
            )
            return
        
        # Extract broadcaster ID and convert to fixed 6-digit code
        broadcaster_id = create_broadcasting.extract_broadcaster_id(channel_id)
        fixed_code = create_broadcasting.convert_to_fixed_code(channel_id)
        
        if not fixed_code:
            bot.edit_message_text(
                "Error generating fixed code. Please try again.",
                call.message.chat.id,
                call.message.message_id
            )
            return
        ...
        try:
            cursor.execute(
                "UPDATE users SET PEER_ID = ?, STATUS = 'BROADCASTER' WHERE USER_ID = ?", 
                (fixed_code, call.from_user.id)
            )
            conn.commit()
            logger.info(f"Updated user {call.from_user.id} as BROADCASTER with PEER_ID {fixed_code} (fixed code from {broadcaster_id})")
        except Exception as db_error:
            logger.error(f"Database error updating user as broadcaster: {db_error}")
        
        # Replace the options message with the generated channel ID and instructions
        # Format the channel ID to prevent it from being treated as a command
        # Add a zero-width space after the slash to prevent command triggering
        formatted_channel_id = channel_id.replace("/", "/​")  # There's a zero-width space after the slash
        
        # Count the number of listeners with the same PEER_ID and notify them
        try:
            ...
            logger.info(f"Broadcaster {call.from_user.id} has {listener_count} listeners")
            
            # Notify all listeners that the broadcaster is live
            for listener in listeners:
                listener_id = listener[0]
                try:
                    bot.send_message(
                        listener_id,
                        "🎉 Hurray!! Your broadcaster is live now!!",
                        disable_notification=False  # Make sure listeners get notified
                    )
                    logger.info(f"Notified listener {listener_id} that broadcaster {call.from_user.id} is live")
                except Exception as notify_error:
                    logger.error(f"Error notifying listener {listener_id}: {notify_error}")
        except Exception as count_error:
            logger.error(f"Error counting/notifying listeners: {count_error}")
            listener_count = 0
        
        bot.edit_message_text(
            f"You have started your broadcasting now, currently you have {listener_count} listeners.\n\nShare this code with your listeners.",
            call.message.chat.id,
            call.message.message_id
        )
        
        # Also send the channel ID as a separate message for easy copying
        bot.send_message(
            call.message.chat.id,
            channel_id,
        ...
        logger.error(f"Error handling broadcasting broadcaster option: {e}")
        bot.send_message(call.message.chat.id, "Error processing request. Please try again later.")

@bot.callback_query_handler(func=lambda call: call.data == "broadcasting_cancel")
def handle_broadcasting_cancel_callback(call):
    """Handle the Cancel option for broadcasting."""
    try:
        ...
        logger.error(f"Error handling broadcasting cancel option: {e}")
        bot.answer_callback_query(call.id, text="Error processing request", show_alert=True)
...
def handle_ai_chat_bot_callback(call):
    bot.answer_callback_query(
    call.id,
    text="⚠️\n\nBella has memory and it can last long for 24hours for silver membership plan!!",
    show_alert=True
)
